...
void Program::zapiszEgzamin(int blok, int odrzucone_1, int odrzucone_2){
    int przedmiot;
    if(m_BazaPytan->getPrzedmiot() == "PPK") przedmiot = 0;
    if(m_BazaPytan->getPrzedmiot() == "PK") przedmiot = 1;
    int numer_odrzuconego_1 = (odrzucone_1 != -1) ? m_WylosPyt[blok][odrzucone_1].getNumer(): -1;
    int numer_odrzuconego_2 = (odrzucone_2 != -1) ? m_WylosPyt[blok][odrzucone_2].getNumer(): -1;
    historia.dodajEgzamin(m_WylosPyt[blok], blok, numer_odrzuconego_1, numer_odrzuconego_2, przedmiot);
}

void Program::serializuj(const QString& sciezka){ historia.serializuj(sciezka); }
std::vector<Egzamin> Program::deserializuj(const QString& sciezka){ return historia.deserializuj(sciezka); }
