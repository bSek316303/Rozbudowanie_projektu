...
void MainWindow::wyborPytan(int blok)
{
    ...
    if(numToRem.size() == 1){
        m_uslugi.zapiszEgzamin(blok, numToRem[0], -1);
    }
    else if(numToRem.size() == 2){
        m_uslugi.zapiszEgzamin(blok, numToRem[0], numToRem[1]);
    }
...
void MainWindow::wypisz(QVector<Pytanie>& pytania, int blok)
{
    if(pytania.size() == MIN_LICZ_PYT) // Jeśli liczba wylosowanych pytań to 3 (wartość domyślna)
    {
        ...
        if(!czyDodPyt[blok]) m_uslugi.zapiszEgzamin(blok, -1, -1);
        wypiszPelnePytania(pytania, blok);
        czyDodPyt[blok] = false;
    }
...
void MainWindow::on_pushButton_clicked()
{
    ...
    QString folder = QFileDialog::getExistingDirectory( this, tr("Wybierz folder zapisu kopii zapasowej"), start_folder, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
